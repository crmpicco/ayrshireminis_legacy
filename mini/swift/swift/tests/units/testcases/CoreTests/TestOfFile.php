...
class TestOfFile extends UnitTestCase
{
  ...
  {
    $file = new Swift_File($GLOBALS["CONF"]->FILES_PATH . "/manchester.jpeg");
    
    $data = "";
    while (false !== $byte = $file->getByte()) $data .= $byte;
    
    $this->assertIdentical($data, file_get_contents($GLOBALS["CONF"]->FILES_PATH . "/manchester.jpeg"));
  }
  /**
  ...
  {
    $file = new Swift_File($GLOBALS["CONF"]->FILES_PATH . "/manchester.jpeg");
    $this->assertIdentical($file->readFull(), file_get_contents($GLOBALS["CONF"]->FILES_PATH . "/manchester.jpeg"));
  }
  /**
