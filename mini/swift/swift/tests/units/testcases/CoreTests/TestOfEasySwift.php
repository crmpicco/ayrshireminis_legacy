...
class TestOfEasySwift extends AbstractTestWithSend
{
  ...
  function testAddingAttachmentsIsHandledByMessageObject()
  {
    ...
    $swift->addAttachment(new Swift_File($GLOBALS["CONF"]->FILES_PATH . "/gecko.png"));
  }
  /**
  ...
  function testAddingImagesIsHandledByMessageObject()
  {
    ...
    $swift->newMessageFromObject($mockMessage);
    $swift->addImage($GLOBALS["CONF"]->FILES_PATH . "/manchester.jpeg");
    $swift->addImage($GLOBALS["CONF"]->FILES_PATH . "/durham.gif");
    $swift->addImage($GLOBALS["CONF"]->FILES_PATH . "/gecko.png");
  }
  /**
  ...
  function testCIDSrcValueIsReturnedWhenAddingImage()
  {
    ...
    $this->assertPattern("/^cid:.+\$/i", $swift->addImage($GLOBALS["CONF"]->FILES_PATH . "/manchester.jpeg"));
    $this->assertPattern("/^cid:.+\$/i", $swift->addImage($GLOBALS["CONF"]->FILES_PATH . "/durham.gif"));
    $this->assertPattern("/^cid:.+\$/i", $swift->addImage(new Swift_File($GLOBALS["CONF"]->FILES_PATH . "/gecko.png")));
  }
  /**
   * Special case, adding images (or embedded files) returns Content-ID
   */
  function testCIDSrcValueIsReturnedWhenAddingEmbeddedFile()
  {
    ...
    $this->assertPattern("/^cid:.+\$/i", $swift->embedFile(file_get_contents($GLOBALS["CONF"]->FILES_PATH . "/manchester.jpeg"), "image/jpeg"));
    $this->assertPattern("/^cid:.+\$/i", $swift->embedFile(new Swift_File($GLOBALS["CONF"]->FILES_PATH . "/durham.gif"), "image/gif", "myimage.gif"));
    $this->assertPattern("/^cid:my_cid123\$/i", $swift->embedFile(file_get_contents($GLOBALS["CONF"]->FILES_PATH . "/gecko.png"), "image/png", "myimage.png", "my_cid123"));
  }
  /**
