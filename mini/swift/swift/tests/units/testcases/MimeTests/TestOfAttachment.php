...
class TestOfAttachment extends UnitTestCase
{
   ...
   */
  function testAttachmentContainsNeededHeaders()
  {
    $attachment = new Swift_Message_Attachment("some string");
    $structure =& $attachment->build();
    ...
    $attachment = new Swift_Message_Attachment(new Swift_File($GLOBALS["CONF"]->FILES_PATH . "/manchester.jpeg"));
    $structure =& $attachment->build();
    $structure = $structure->readFull();
    $this->assertPattern(
      "~Content-Type: application/octet-stream;\\s* name=(\"?)(.*?)\\1\r\nContent-Transfer-Encoding: base64\r\n".
   ...
   */
  function testFileNameCanBeChanged()
  {
   ...
   */
  function testFileNameCanBeReadFromFileStream()
  {
    $attachment = new Swift_Message_Attachment(new Swift_File($GLOBALS["CONF"]->FILES_PATH . "/manchester.jpeg"));
    ...
    $attachment->setData(new Swift_File($GLOBALS["CONF"]->FILES_PATH . "/manchester.jpeg"));
    $this->assertEqual("manchester.jpeg", $attachment->getFileName());
    $structure =& $attachment->build();
    $structure = $structure->readFull();
    $this->assertPattern(
   ...
   */
  function testFileNameCanBeOverriddenEvenWhenUsingFileStream()
  {
    $attachment = new Swift_Message_Attachment(new Swift_File($GLOBALS["CONF"]->FILES_PATH . "/manchester.jpeg"), "foo.bar");
   ...
   */
  function testDescriptionCanBeOverridden()
  {
    $attachment = new Swift_Message_Attachment("some string", "my_file.txt");
   ...
   */
  function testSettingFileNameResetsDescription()
  {
    $attachment = new Swift_Message_Attachment("some string", "my_file.txt");
    $attachment->setDescription("another_file.txt");
   ...
   */
  function testPrefixIsUsedInFileName()
  {
   ...
   */
  function testSequenceNumberIsAlwaysIncrementedInFileName()
  {
